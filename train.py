...
def parse_args():
    ...
    p.add_argument("--data_file",   default=None,
                   help="Path to pre-tokenized .pt file (from prepare_data_phase1b.py). "
                        "Skips all HuggingFace downloading — GPU starts training immediately.")
...
class TagAwareDataset(Dataset):
    ...
    def _weights(self, tokens: torch.Tensor) -> torch.Tensor:
        ...
        for i, tok in enumerate(tokens.tolist()):
            ...
            base_w = 0.0 if state == "user" else (self.tw if state == "think" else 1.0)
            # FIX: give EOS and </assistant> tokens 2x weight so the model learns
            # to stop reliably. This prevents runaway generation & </s> spam.
            stop_boost = 2.0 if tok in (self.eos, self.think_end) else 1.0
            w[i] = base_w * stop_boost if base_w > 0.0 else 0.0
        return w
...
def load_sft_data(tokenizer, seq_len, think_weight, cap=None) -> TagAwareDataset:
    ...
    # ── 0. Identity seeds (repeated to anchor model's self-knowledge) ─────────
    # Without these, the model will hallucinate a human identity ("I'm from NYC...")
    print("🪪 Injecting NovaMind identity seeds...")
    IDENTITY_SEEDS = [
        ("Who are you?",
    ...
    # Repeat identity seeds MANY times so they completely override pre-training human identity.
    # 20 was not enough (drowned by 5B tokens of human text). 500x anchors it strongly.
    REPEAT = 500
    for q, a in IDENTITY_SEEDS:
        for _ in range(REPEAT):
            texts.append(wrap(q, a))
    print(f"   ✅ {len(IDENTITY_SEEDS) * REPEAT:,} identity seed examples (x{REPEAT} repeats)")

    # ── 1. Alpaca (52K instruction-following examples, downloadable) ──────────
    ...
    for r in alpaca:
        ...
        if r["input"].strip():
            u += f"\n\n{r['input'].strip()}"
        ans = r["output"].strip()
        if not ans or len(ans) < 5:  # skip empty/junk answers
            continue
        texts.append(wrap(u, ans))
    print(f"   ✅ {len(texts) - n_before:,} Alpaca examples")

    # ── 2. OpenAssistant OASST1 (88K human conversations, downloadable) ───────
    # Replaces ShareGPT which was streaming-only and caused the GPU credit waste.
    # OASST1 is high quality: human annotators rating conversations.
    print("💬 Loading OpenAssistant OASST1 (replaces ShareGPT)...")
    try:
        oasst = load_dataset("OpenAssistant/oasst1", split="train")
        # Build message map: id → row
        msg_map = {row["message_id"]: row for row in oasst}
        n_before = len(texts)
        oasst_cap = min(cap, 60_000) if cap else 60_000
        count = 0
        for row in oasst:
            if count >= oasst_cap:
                break
            # Only take assistant replies that have a prompter parent
            ...
            if not q or not ans or len(ans) < 10:
                continue
            # Quality filter: skip very low-rated responses
            if row.get("rank", 0) is not None and row.get("rank", 0) > 3:
                continue
            texts.append(wrap(q, ans))
            count += 1
        print(f"   ✅ {len(texts) - n_before:,} OASST1 examples")
    except Exception as e:
        print(f"   ⚠️  OASST1 unavailable ({e}), skipping.")

    # ── 3. Dolly 15K (15K human-written instructions, high quality) ───────────
    print("💬 Loading Databricks Dolly 15K...")
    try:
        dolly = load_dataset("databricks/databricks-dolly-15k", split="train")
        n_before = len(texts)
        dolly_cap = min(cap, 15_000) if cap else 15_000
        for i, row in enumerate(dolly):
            ...
            if not ans or len(ans) < 10:
                continue
            texts.append(wrap(q, ans))
        print(f"   ✅ {len(texts) - n_before:,} Dolly examples")
    except Exception as e:
        print(f"   ⚠️  Dolly unavailable ({e}), skipping.")

    # ── 4. DailyDialog (conversational back-and-forth) ─────────────────────────
    ...
    try:
        ...
        if cap: dd = dd.select(range(min(cap // 4, len(dd))))
        n_before = len(texts)
        for item in dd:
            ...
            if t: texts.append(t)
        print(f"   ✅ {len(texts) - n_before:,} DailyDialog examples")
    except Exception as e:
        print(f"   ⚠️  DailyDialog unavailable ({e}), skipping.")

    print(f"\n   📊 Total SFT examples: {len(texts):,}")
...
def main():
    ...
    if args.phase == "pretrain":
        if getattr(args, 'data_file', None) and Path(args.data_file).exists():
            print(f"   ⚡ Loading pre-tokenized data from disk: {args.data_file}")
            chunks = torch.load(args.data_file, map_location="cpu", weights_only=True).long()
            print(f"   ✅ {len(chunks):,} chunks ({len(chunks)*args.max_seq_len/1e9:.2f}B tokens) — no download needed!")
        else:
            if getattr(args, 'data_file', None):
                print(f"   ⚠️  data_file not found: {args.data_file} — falling back to download")
            chunks = load_pretrain_data(tokenizer, args.max_seq_len, cap)
        dataset = PretrainDataset(chunks)
    elif args.phase == "sft":
        if getattr(args, 'data_file', None) and Path(args.data_file).exists():
            # FIX: SFT phase now supports --data_file (pre-tokenized on CPU, saves H100 time)
            print(f"   ⚡ Loading pre-tokenized SFT data from disk: {args.data_file}")
            chunks = torch.load(args.data_file, map_location="cpu", weights_only=True).long()
            print(f"   ✅ {len(chunks):,} chunks ({len(chunks)*args.max_seq_len/1e9:.2f}B tokens)")
            dataset = TagAwareDataset(
                chunks, think_weight,
                _tag_id(tokenizer, "<think>"), _tag_id(tokenizer, "</think>"),
                _tag_id(tokenizer, "<assistant>"), _tag_id(tokenizer, "<human>"),
                tokenizer.eos_token_id,
